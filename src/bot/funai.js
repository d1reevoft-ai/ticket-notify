...
10. АБСОЛЮТНО ЗАПРЕЩЕНО выводить пользователю служебные теги ([ДОСЬЕ СОБЕСЕДНИКА], [fact], [MACRO] и т.д.). Используй эти блоки незаметно под капотом, выдавая только финальный человеческий ответ.`;
...
3. НИКОГДА не пиши фразы типа "В памяти я нашёл...", "Согласно базе знаний...", "Вот что мне удалось найти...". Просто отвечай на вопрос, как будто ты сам это знаешь.
4. Будь КРАТКИМ. Если в памяти есть прямой ответ из одного слова — ответь одним словом. Не расписывай лишнего.
5. Если вопрос про правила — цитируй точный текст правила.
6. Если вопрос сложный или неоднозначный — НЕ ОТВЕЧАЙ, скажи "Секунду, уточню у коллеги".
7. Твой стиль — живой человек (модератор), а не вежливая нейросеть-помощник. Избегай длинных вводных фраз.`;
...
class FunAI {
    ...
    _handleWidgetCommand(normalized, context) {
        // ── Greetings ──
        const greetingsRegex = /^(привет|здравствуй|здравствуйте|хай|ку|добрый день|добрый вечер|приветствую|здарова|салам|hello|hi)[\s!.,]*(бот|фунай|funai)?[\s!.,]*$/i;
